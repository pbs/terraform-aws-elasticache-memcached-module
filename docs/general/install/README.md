...
```hcl
module "memcached" {
    source = "github.com/pbs/terraform-elasticache-memcached-module?ref=x.y.z"
}
```
...
```bash
mkdir -p 'terraform/modules/elasticache-memcached'
gh -R 'pbs/terraform-elasticache-memcached-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/elasticache-memcached'
rm -f release.tar.gz
```
...
```bash
git remote add -f elasticache-memcached git@github.com:pbs/terraform-elasticache-memcached-module.git
git subtree add --prefix terraform/modules/elasticache-memcached elasticache-memcached main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/elasticache-memcached elasticache-memcached $REF --squash
```
